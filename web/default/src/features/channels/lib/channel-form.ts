...
export const channelFormSchema = z
  .object({
    ...
    responses_via_chat_enabled: z.boolean().optional(), // OpenAI-compatible upstreams without native Responses
...
export function transformChannelToFormDefaults(
  ...
  let responsesViaChatEnabled = false
  ...
  if (channel.settings) {
    try {
      ...
      responsesViaChatEnabled = parsed.responses_via_chat_enabled === true
  ...
  return {
    ...
    responses_via_chat_enabled: responsesViaChatEnabled,
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  if (formData.type === 1) {
    ...
    settingsObj.responses_via_chat_enabled =
      formData.responses_via_chat_enabled === true
    ...
    if ('responses_via_chat_enabled' in settingsObj)
      delete settingsObj.responses_via_chat_enabled
